...
def create(conf):
    ...
    with tf.variable_scope(name):
        ...
        res = convolution2d(res, num_outputs=16, kernel_size=(8, 8), stride=(4, 4), activation_fn=tf.nn.relu,
                            weights_initializer=tf.truncated_normal_initializer(stddev=0.05))
        print 'conv1 shape', res.get_shape()
        conv1 = res

        res = convolution2d(res, num_outputs=32, kernel_size=(4, 4), stride=(2, 2), activation_fn=tf.nn.relu,
                            weights_initializer=tf.truncated_normal_initializer(stddev=0.05))
        ...
        res = fully_connected(res, 256, activation_fn=tf.nn.relu,
                              weights_initializer=tf.truncated_normal_initializer(stddev=0.05))
        fc1 = res
        res = fully_connected(res, actions_n, activation_fn=lambda a:a,
                              weights_initializer=tf.truncated_normal_initializer(stddev=0.05))

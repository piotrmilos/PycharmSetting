...
class GenerousAgent(Agent):
    ...
    gets him the least stones possible"""
    ...
    def get_action(self, state, legal_moves):
        ...
        move = self.generous_search(state)
        return move   
        
    def generous_search(self,state):
        ...
        best_move = sorted(option_value_list,key=lambda x: x[0], reverse=True)[-1]

...
class BufferedProcessor(object):
    def __init__(self, chunk_loader, buffer_size, add_to_queue_func, name):
        ...
        self.name = name
        self.buffer = mp.Queue(maxsize=self.buffer_size)
    ...
    def get_iterator(self):
        ...
        process = Process(target=reader_process, args=(self.chunk_loader, self.buffer, self.add_to_queue_func))
        process.start()
        ...
        while True:
            ...
            try:
                v = self.buffer.get(timeout=TIMEOUT_IN_SECONDS)
            except Empty:
...
# TODO: tidy this Output director thing
class SimpleOutputDirector(object):
    def __init__(self, mb_size, output_partial_batches=False):
...
class MultiprocessingChunkProcessor(object):
    ...
    def get_iterator(self):
        ...
        for chunk in chunks(self.elements_to_process, self.chunk_size):
            chunk_results = pool.map(self.process_func, chunk)
...
def get_iterator(process_func, elements_to_process, output_director, pool_size=4, buffer_size=20, chunk_size=100):
    ...
    buffered_processor = BufferedProcessor(
    ...
    return buffered_processor.get_iterator()
